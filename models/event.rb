...
class Event
  ...
  def content_html
    parse_content.to_html.html_safe
  end
  ...
  def summary_html
    if metadata[:summary].present?
      Maruku.new(metadata[:summary]).to_html.html_safe
    else
  ...
  def image_url
    "/event_images/#{image.split("/").last}" if image
  end
  ...
  class << self
    def all
      @@posts ||= Dir.glob(LockIn.root + "/../events/*.md").map do |filename|
        Event.new filename
